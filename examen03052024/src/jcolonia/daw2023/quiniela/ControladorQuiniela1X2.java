...
public class ControladorQuiniela1X2 {
	 ...
	 * @param scanner Escáner para la entrada de datos.
	 */
	public void buclePrincipal(Scanner scanner) {
	    VistaMenú menú;
	    int opción;
	    int n = 0;
	    boolean salir = false;

	    menú = new VistaMenú("Quiniela 1-X-2", TXT_MENÚ_PRINCIPAL, scanner);
	    ...
	    do {
	        ...
	        } catch (DatoPartido1X2Exception | Partido1X2Exception e) {
	 ...
	 * @throws Partido1X2Exception Si ocurre un error al introducir un dato.
	 * @throws DatoPartido1X2Exception Si los datos introducidos no siguen los patrones para su añadido.
	 */
	...
	private void listado() {
		VistaListado dlg;
		 dlg = new VistaListado("Lista de Resultados", listaResultados.generarListado(), new Scanner(System.in));
	    dlg.listar();
	 ...
	 * @throws Partido1X2Exception cuando los datos no son correctos.
	 */
	...
	private void importación(String rutaArchivo) {
		...
		}catch (Partido1X2Exception ex) {
			System.err.println("Error al importar el partido: " + ex.getMessage());
		}
	}
	/**
